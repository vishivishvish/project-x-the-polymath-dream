...
## 19. **CS031: Robotics Sim — Rediscovering Control Principles from Simulation via Progressive Model Intelligence**
**Disciplines:** Robotics & Ground-based Autonomous Systems Engineering (Tier 5) × Evolutionary Computation × Deep Learning & RL × Graph Learning × LLM/Generative AI & Agentic AI (Tier 2)
**Hook:** Start with a simple planar 2-DOF arm (reach task) or differential-drive robot (navigate task) in a lightweight Python simulator (Gymnasium classic/robotics, PyBullet-light, or pure NumPy/pymunk). Train a progression of models — from linear regression to graph neural policies to transformer-based world models to an agentic AI that *designs* its own curriculum — each rediscovering known control principles (PID, LQR, MPC, impedance control, sim-to-real gaps) from raw sensorimotor streams. The "aha" plot: a performance ladder showing how each layer of the House Stack closes the gap to optimal control, culminating in a hybrid Agentic+Neural+Graph+Evolutionary system that outperforms hand-tuned baselines.
**House fit:** Pure rediscovery of control theory from data (like CS001 rediscovers radiobiology); domain story (robot learning as compressed optimal control); beautiful visualizations (phase portraits, policy heatmaps, curriculum trees); minimal supervision with clear metrics (success rate, energy, sample efficiency).
**CPU feasibility:** **Easy–Medium** — recommended stacks:
  - **Primary (Easy):** Gymnasium `Pendulum-v1`, `Reacher-v2`, `FetchReach-v2` (MuJoCo-free) — pure Python, trains in seconds/minutes on CPU.
  - **Primary (Easy):** Custom 2-DOF planar arm / differential-drive in pure NumPy + `pymunk` (lightweight 2D physics) — zero dependencies, full control.
  - **Stretch (Medium):** PyBullet (`pybullet_envs` or `gymnasium[pybullet]`) — heavier but still CPU-feasible for small robots (KUKA, Humanoid subsets); MuJoCo via `gymnasium[mujoco]` needs `mujoco` license (free) but runs on CPU with ~2-5x slowdown vs GPU.
**Novelty:** Most robotics ML benchmarks compare *algorithms* on fixed tasks; this compares *progressive intelligence layers* (House Stack) on the *same* task, showing what each layer buys. Directly hits Tier 5 (Robotics) and Tier 2 (DL/RL, Evo Comp, Graph Learning, Agentic AI) — the full intelligence core.
**House stack fit:**
- **L1 Supervised+Unsupervised:** Supervised — behavior cloning from expert trajectories (MPC/oracle); Unsupervised — cluster state-action trajectories by strategy (reach vs. oscillate vs. stuck); anomaly detection for sim-to-real transfer gaps.
- **L2 XAI:** SHAP on policy network (which state dims drive action); intrinsic — linear/tabular policy coefficients = interpretable gains; attention on transformer world model = salient state regions; policy decision boundaries visualized in phase space.
- **L3 Deep Learning:** MLP policy (2-layer, ~10k params) → LSTM/GRU for history dependence → 1D CNN for proprioceptive sequences → Graph NN (GNN) on robot kinematic tree (links as nodes, joints as edges) — all CPU <10 min.
- **L4 Foundation Models:** **CPU skip honest** — no robotics foundation model on CPU. Use **Decision Transformer** (small, ~1M params, frozen, ONNX) pre-trained on D4RL for offline RL initialization; or **RT-1/X** embeddings (frozen) if available via ONNX.
- **L5 GenAI:** Conditional VAE / Diffusion (small, ~50k params) on state-action trajectories → generate diverse expert demonstrations for data augmentation; World Model (Dreamer-v3 style, small) → imagine rollouts for planning; GFlowNet for diverse skill discovery.
- **L6 GenAI Features:** World model latent states → feed into policy as enriched observations; Diffusion-generated trajectories → augment behavior cloning dataset; GNN embeddings of kinematic tree → transfer across morphologies (ablation: with vs. without).
- **L7 Agentic:** **Structured** — Curriculum agent: designs task progression (reach → reach+obstacle → grasp-lite → manipulate) based on policy competence map; Active data agent: queries "most informative initial state" for expert rollout. **Autonomous** — Meta-agent: writes reward functions, chooses architecture (MLP vs. GNN vs. Transformer), tunes hyperparameters via evolutionary search, and *proposes* sim modifications (add friction, delay, noise) to close sim-to-real gap — all with human-in-the-loop approval checkpoints.
- **Hybrid Endgame (Agentic + Neural + Graph + Evolutionary):** Evolutionary search (NEAT/CMA-ES) over *policy architectures* (MLP, LSTM, GNN, Transformer) + *hyperparameters* + *reward shaping*, guided by an LLM agent that proposes mutations ("try adding a graph attention layer over the kinematic tree," "add a world-model auxiliary loss"). The agent maintains a "control knowledge base" (rediscovered PID gains, LQR matrices, impedance profiles) and injects them as inductive biases. Result: a self-improving control stack that rediscovers classical control, then surpasses it by learning residual policies on top of model-based priors.
- **Rediscovery thread:** L1 rediscovers state-space clustering (reach vs. fail modes); L2 reveals *which* states matter (joint angles vs. velocities vs. end-effector pos); L3 learns feedback policies that approximate LQR; L4 transfers offline RL priors; L5 generates diverse skills & imagines futures; L6 fuses kinematic structure + imagined data into policy; L7 designs the curriculum that teaches the robot to walk before it runs; Hybrid endgame = agent that *invents* the next layer of control intelligence.

---

## 20. **CS032: Self-Driving Car Sim — Rediscovering Driving Intelligence from Endless Ribbon Roads**
**Disciplines:** Robotics & Ground-based Autonomous Systems Engineering (Tier 5) × Deep Learning & RL × Evolutionary Computation × Graph Learning × LLM/Generative AI & Agentic AI (Tier 2) × Information Theory (Tier 2)
**Hook:** A minimal, fully-Python "endless ribbon road" — a procedurally generated, never-ending twisting track with lane markers, rendered as a forward-facing camera view (Pygame/NumPy or Gymnasium `CarRacing-v3` as drop-in). An agent learns to steer, accelerate, and brake from raw pixels (or lightweight state: curvature, offset, speed). Progress through the House Stack: behavioral cloning from a PID "expert" → RL (PPO/SAC) on reward shaped by lane-centering → Transformer world model predicting future road geometry → GenAI synthesizing novel road/weather conditions → Structured vs. Autonomous driving agents (planner vs. end-to-end) → Hybrid endgame fusing Agentic reasoning + Neural policy + Graph road topology + Evolutionary search over architectures/rewards. The "aha" plot: a performance ladder (lane deviation, speed, safety interventions) showing each layer closing the gap to human-like smooth driving, culminating in a self-improving driver that *invents* its own curriculum of road scenarios.
**House fit:** Pure rediscovery of driving principles (racing line, lookahead, smooth control) from raw sensorimotor data (like CS001 rediscovers radiobiology); domain story (driving as real-time optimal control under uncertainty); beautiful visualizations (policy heatmaps on track, attention on road features, curriculum trees, generated road variants); minimal supervision with clear metrics (laps completed, jerk, lane deviation, intervention rate).
**CPU feasibility:** **Easy–Medium** — recommended stacks:
  - **Primary (Easy):** Gymnasium `CarRacing-v3` (Box2D-based, pure Python, no MuJoCo/Unity) — trains in minutes on CPU; discrete or continuous actions; top-down or forward-facing view.
  - **Primary (Easy):** Custom "Endless Ribbon Road" in pure NumPy + Pygame — procedural spline track, lane-centered reward, 64×64→3×64×64 RGB or 16-dim state (curvature ahead, lateral offset, heading error, speed). Zero heavy deps, full control, trains in seconds/minutes.
  - **Stretch (Medium):** DonkeyCar sim (Python, TensorFlow) — more realistic car dynamics but heavier; Udacity simulator (Unity) — **avoid** (not Pythonic, GPU-needed).
**Novelty:** Most self-driving benchmarks (CARLA, nuScenes, Waymo) are data-heavy, GPU-bound, and fixed-task. This frames driving as a *progressive intelligence ladder* (House Stack) on a *minimal, generative* task — showing what each layer buys, from PID to agentic curriculum design. Directly hits Tier 5 (Robotics/Ground Autonomous) and full Tier 2 (DL/RL, Evo Comp, Graph Learning, Agentic AI, Info Theory).
**House stack fit:**
- **L1 Supervised+Unsupervised:** Supervised — behavioral cloning from PID/MPC expert on generated tracks; Unsupervised — cluster driving segments by maneuver type (straight, left-turn, right-turn, chicane, recovery); anomaly detection for OOD road geometries (sharp hairpins, adverse camber).
- **L2 XAI:** SHAP on policy (which pixels/state dims drive steering); intrinsic — linear policy on hand-crafted features = interpretable gains (lookahead distance, curvature gain); attention on Transformer world model = salient road features (apex, vanishing point); decision boundaries in (curvature, speed) space.
- **L3 Deep Learning:** MLP policy (2-layer, ~20k params) → LSTM/GRU for temporal integration → 1D CNN on sequence of curvature/offset → Transformer encoder (small, ~200k params) on tokenized road segments → Graph NN on road graph (nodes = waypoints, edges = connectivity, curvature) — all CPU <15 min.
- **L4 Foundation Models:** **CPU skip honest** — no driving foundation model on CPU. Use **Decision Transformer** (small, frozen, ONNX) pre-trained on offline driving datasets (e.g., D4RL `carla-lane`) for initialization; or **DriveGPT/GAIA-1** embeddings (frozen, ONNX) if available.
- **L5 GenAI:** Conditional VAE / Diffusion (small, ~100k params) on road geometry tokens → generate infinite novel track layouts (curvature profiles, elevation, width); World Model (Dreamer-style, small) → imagine rollouts for planning; GFlowNet for diverse driving styles (aggressive, conservative, fuel-efficient); Text-to-track LLM (small, LoRA) → "generate a rainy mountain pass with hairpins" → synthetic curriculum.
- **L6 GenAI Features:** World model latent → enriched observation for policy; Generated track embeddings → condition policy on road type (highway vs. mountain); Graph embeddings of road topology → transfer across tracks (ablation: with vs. without); Synthetic weather/lighting augmentations (GenAI) → robustness.
- **L7 Agentic:** **Structured** — Curriculum agent: progresses track difficulty (straight → gentle curves → chicanes → hairpins → variable width → weather) based on competence map; Safety agent: monitors lane deviation/jerk, triggers "intervention" (human or recovery policy). **Autonomous** — Meta-agent: writes reward functions (lane-centering vs. speed vs. comfort), chooses architecture (CNN vs. Transformer vs. GNN), tunes hyperparameters via evolutionary search, proposes sim modifications (sensor noise, actuation delay, tire model) to close sim-to-real gap — human-in-the-loop approval.
- **Hybrid Endgame (Agentic + Neural + Graph + Evolutionary):** Evolutionary search (CMA-ES/NEAT) over *policy architectures* + *reward weights* + *curriculum schedules*, guided by an LLM agent that proposes mutations ("add graph attention over waypoint graph," "add world-model consistency loss," "shape reward for smooth jerk"). Agent maintains a "driving knowledge base" (rediscovered racing line, lookahead distance, PID gains for lateral/longitudinal control) and injects them as inductive biases (e.g., initialize policy with PID, then learn residual). Result: a self-improving driving stack that rediscovers classical control (PID, pure pursuit, MPC), then surpasses it by learning context-aware residuals on top of model-based priors — and *generates its own test tracks* to verify generalization.
- **Rediscovery thread:** L1 rediscovers maneuver clusters (turn types, recovery modes); L2 reveals *what the driver looks at* (vanishing point, apex, tangent); L3 learns feedback policies approximating pure pursuit / MPC; L4 transfers offline driving priors; L5 generates infinite roads & imagines futures; L6 fuses road topology + imagined rollouts into policy; L7 designs the curriculum that teaches the car to cruise before it races; Hybrid endgame = agent that *invents* the next layer of driving intelligence and the tracks to test it on.